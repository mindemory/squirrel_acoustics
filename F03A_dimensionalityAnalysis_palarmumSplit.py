...
from params import PROJECT_PATH, acoustic_features, species_colors
...
plot_projection_complex(
    X_pca, y, loadings_pca, selected_features,
    "PCA Projection (All Features)",
    f"PC1 ({pca.explained_variance_ratio_[0]*100:.1f}%)",
    f"PC2 ({pca.explained_variance_ratio_[1]*100:.1f}%)",
    os.path.join(save_dir, 'pca_all_features.png'),
    wspace=0.05, 
    ylim=(-4, 9), # Adjusted ylim for wider spread with all features
    legend_loc='upper right'
)
print(f"Saved Figure 3D to: pca_all_features.png")


# --- UMAP (Figure 3E) ---
print("\nGenerating Figure 3E (Supervised UMAP with All Features)...")
try:
    import umap
    ...
    umap_emb = umap.UMAP(n_neighbors=50, min_dist=0.5, n_components=2, random_state=42).fit_transform(X_selected, y=y_encoded)
    ...
    plt.xlabel("UMAP 1")
    plt.ylabel("UMAP 2")
    plt.title(f"Supervised UMAP Projection (All Features)")
    # Remove ticks/spines for "nicer" look
    plt.xticks([])
    plt.yticks([])
    sns.despine() # Remove box
    plt.tight_layout()
    filename_3e = 'umap_all_features.png'
...
# --- Boxplots & Statistics (Figure 3F) ---
print(f"\nGenerating Boxplots and Stats for All Features: {selected_features}")
# This generates Figure 3F
plot_feature_boxplots(master_random_df, selected_features, 'Species', save_dir)
perform_statistics(master_random_df, selected_features, 'Species', save_dir)
