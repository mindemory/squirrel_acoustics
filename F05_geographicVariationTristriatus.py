...
# --- Configuration ---
SPECIES_NAME = 'F. tristriatus'
MIN_FILES_PER_LOCATION = 10
SEED = 42

# User-Requested Location Order
LOCATION_ORDER = ['Thrissur', 'Malappuram', 'Wayanad', 'Goa', 'Ratnagiri', 'Pune']

# Define Fixed Colors (using tab10)
# Map first 6 colors of tab10 to the 6 locations
TAB10 = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']

LOCATION_COLORS = {
    'Thrissur': TAB10[0],    # Blue
    'Malappuram': TAB10[1],  # Orange
    'Wayanad': TAB10[2],     # Green
    'Goa': TAB10[3],         # Red
    'Ratnagiri': TAB10[4],   # Purple
    'Pune': TAB10[5]         # Brown
}
...
def plot_feature_boxplots(df, features, target_col, output_dir, location_order, location_colors):
    # Simplified Boxplots: No significance bars
...
print(f"Final Location Order: {LOCATION_ORDER}")

save_dir = os.path.join(PROJECT_PATH, 'Figures', 'geo_var_tristriatus')
...
for k in n_features_list:
    ...
    accuracy_scores.append(accuracy_score(y_te, y_pr))
    f1_scores.append(f1_score(y_te, y_pr, average='weighted'))
...
if X_lda.shape[1] == 2:
    plot_projection_complex(
        ...
        wspace=0.3, legend_loc='upper left',
        xlim=(-8, 5), ylim=(-5, 5)
    )

# --- 4. PCA (X -5 to 5, Y -2 to 4) ---
...
plot_projection_complex(
    ...
    LOCATION_ORDER, LOCATION_COLORS,
    wspace=0.05, legend_loc='upper right',
    xlim=(-5, 5), ylim=(-2, 4)
)
...
print(f"\n=== F04B {SPECIES_NAME} Analysis Complete ===")
