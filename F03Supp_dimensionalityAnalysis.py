...
print("Loading Data for Supplementary (Full Dataset)...")
df_folder = os.path.join(PROJECT_PATH, 'dataframes')
# Using master_df.csv (Full Dataset) per request
...
X_all = master_df[acoustic_features]

save_dir = os.path.join(PROJECT_PATH, 'Figures', 'dimred_supp')
...
if X_all.isna().any().any():
    ...
    master_df = master_df.loc[valid_indices]
...
plt.title('Random Forest Classifier Permutation Importance (Supp)')
...
plt.title('Random Forest Feature Selection (Supp)') 
plt.xlabel('#Features') 
...
plot_projection_complex(
    ...
    "LDA Projection (Top 6 Features - Supp)",
...
plot_projection_complex(
    ...
    "PCA Projection (Top 6 Features - Supp)",
...
try:
    ...
    plt.xlabel("UMAP 1")
    plt.ylabel("UMAP 2")
    plt.title(f"Supervised UMAP Projection (Top 6 Features - Supp)")
...
plot_feature_boxplots(master_df, top_6_features, 'Species', save_dir)
perform_statistics(master_df, top_6_features, 'Species', save_dir)
